...
#judge_Sign2=1 成功数据
                ...
                judge_Sign2,R_img= yolo.detect_imageback(photoName,cropped_img)#此处存在问题，需要优化
                cv.imwrite('./Result/'+photoName,R_img)
                if(judge_Sign2==7):
                    print('未知错误')
                    full_json={ "code": 7,
                                 "msg":"Unknown error ",
...
#需不需要矫正到这么大的尺寸
                    ...
                    c_img='./cut/'
                    createfile(c_img)                    
                    cv.imwrite(c_img+photoName,image)
                    
#                    B_J_img='./Result/'
#                    createfile(B_J_img)
#                    tbj=time.time()
                    biaoji_img,info_Card,QR=bianji(image,photoName)
