...
def loadfile():
    ...
    labels = []
    # 拿到图像数据路径，方便后续读取
    imagePaths = sorted(list(list_images('./set4')))
    random.seed(42)
...
#    # 绘制结果曲线
    ...
    plt.legend()
    plt.savefig('./output1/smoke_cnn1024.png')
    #
    # 保存模型
    print("------正在保存模型------")
    model.save('./output1/smoke_cnn1024.model')
    f = open('./output1/smoke_cnn1024.pickle', "wb")
